...
export const createTestFilePath = (
    file: vscode.Uri,
    workspaceRoot: string,
    settings: ITestFileSettings
): string => {
...
const createTestFileName = (
    file: ParsedPath,
    settings: ITestFileSettings
): string => {
    let fileNamePattern = settings.fileNamePattern || `test${ file.ext }`;
    return fileNamePattern
        .replace(fileNamePlaceholder, file.name)
        .replace(fileExtPlaceholder, file.ext);
};

const createTestFileDir = (
    file: ParsedPath,
    workspaceRoot: string,
    settings: ITestFileSettings
): string => {
