...
const createImportPart = (fileName: string, exports: IFileExport[], importType: ImportType): string => {
...
const createIndividualImportLine = (fileName: string, exports: IFileExport[]): string => {
    return stringifyExports(fileName, exports);
};

const createGroupedImportLine = (fileName: string): string => {
...
const formatVariableName = (fileName: string) => {
    return (
        fileName
            ...
            .map((s, i) =>
                i !== 0 ? `${s[0].toLocaleUpperCase()}${s.slice(1).toLocaleLowerCase()}` : s.toLocaleLowerCase()
            )
